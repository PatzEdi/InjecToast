...
class Search:
	#First step:
	def initializeSmaliSnippetSearch():
		currentdir = os.getcwd()
		directory = fileOperands.findFile("SmaliInjectionSnippet.txt", currentdir)
...
class Inject:
	...
	def getSmaliInjectContents(toastMessage, SmaliInjectionFileDirectory):#2
		#First step, write the wanted Toast message inside the actual smali code:
		f = open(SmaliInjectionFileDirectory, "r")
		#Get the contents:
		contents = f.read()
		#Replace
...
class Run:
	...
	#		codeInject = input("Toast message to inject: ")
			...
			smaliinjectionsnippetdir = Search.initializeSmaliSnippetSearch()
			smaliInjectCode = Inject.getSmaliInjectContents(toastMessage, smaliinjectionsnippetdir)
